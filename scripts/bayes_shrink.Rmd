...
#Run MCMC
blasso_out = gibbs_blasso(X,y, priors, 1000 , w_init, sigma_init)
```
...
```{r bayesreg}
library(bayesreg)
fit <- bayesreg(lpsa~., train_df, model = "gaussian", prior = "lasso", n.samples = 1000)
```
...
df = data.frame(w1 = c(blasso_out$w[,2], fit$beta[1,]), w2 = c(blasso_out$w[,7], fit$beta[6,]), 
